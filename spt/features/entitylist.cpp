...
private:
	DECL_HOOK_THISCALL(void, CBaseEntityList__RemoveEntity, void*, CBaseHandle handle);
...
namespace patterns
{
	...
	PATTERNS(CBaseEntityList__RemoveEntity, "5135", "8B 44 24 ?? 25 FF 0F 00 00 89 44 24 ?? E9 ?? ?? ?? ??");
} // namespace patterns
...
void EntityList::InitHooks()
{
	...
	HOOK_FUNCTION(server, CBaseEntityList__RemoveEntity);
}
...
IMPL_HOOK_THISCALL(EntityList, void, CBaseEntityList__RemoveEntity, void*, CBaseHandle handle)
{
	if (handle == INVALID_EHANDLE_INDEX)
		Warning("CBaseEntityList::RemoveEntity: Got INVALID_EHANDLE_INDEX!\n");
	return spt_entitylist.ORIG_CBaseEntityList__RemoveEntity(thisptr, handle);
}
